...
function getAccurateLinearIntensity( const bitmap:PBitmap32 ;  const x, y : Single) : Single ;
...
function getValidROI( const bitmap : PBitmap32;  const RegionOfInterest : TRect) : TRect; overload;
function getValidROI( const bitmap : PBitmap32;  const RegionOfInterest : TFRect) : TFRect; overload;
...
begin
  ...
  DrawDisk(img, img.Width, height1_2+height1_2 div 4, height1_2 div 4, clRed32, 1) ;
  Result:=img;
end;
...
Begin
  if false Then Begin
    ix := ceil(x-0.5) ;
    iy := ceil(y-0.5) ;
    i00 := Intensity(getPixel(bitmap, ix, iy)) ;
    i10 := Intensity(getPixel(bitmap, ix+1, iy)) ;
    i01 := Intensity(getPixel(bitmap, ix, iy+1)) ;
    i11 := Intensity(getPixel(bitmap, ix+1, iy+1)) ;
    b := x-ix+0.5 ; a := 1-b ;
    d := y-iy+0.5 ; c := 1-d ;
    b := x-ix+0.5 ; a := 1-b ;
    d := y-iy+0.5 ; c := 1-d ;
  ...
  result := (i00*a+i10*b)*c + (i01*a+i11*b)*d ;
End ;
...
function getValidROI( const bitmap : PBitmap32;  const RegionOfInterest : TFRect) : TFRect;
var
  validROI : TRect;
begin
  validROI.Left := Round( RegionOfInterest.Left );
  validROI.Top := Round( RegionOfInterest.Top );
  validROI.Right := Round( RegionOfInterest.Right );
  validROI.Bottom := Round( RegionOfInterest.Bottom );
  validROI := getValidROI( bitmap, validROI );
  Result.Left := validROI.Left;
  Result.Top := validROI.Top;
  Result.Right := validROI.Right;
  Result.Bottom := validROI.Bottom;
end;
...
begin
  with RegionOfInterest do Begin
    if ((Left=0) and (Top=0) and (Right=0) and (Bottom=0)) or 
		   ((Left=-1) and (Top=-1) and (Right=-1) and (Bottom=-1)) then begin
      validROI.Left:=0;
